...
import br.com.frotasPro.api.modules.logistica.dto.request.ParadaCargaRequest;
...
import static br.com.frotasPro.api.modules.logistica.mapper.ParadaCargaMapper.toResponse;
...
public class AtualizarParadaCargaService {
    ...
    public ParadaCargaResponse atualizar(UUID id, ParadaCargaRequest request) {
        ...
        if (carga.getKmFinal() != null) {
            throw new BusinessException("Não é possível alterar paradas de uma carga já finalizada.");
        }

        parada.setTipoParada(request.getTipoParada());
        parada.setDtInicio(request.getDtInicio());
        parada.setDtFim(request.getDtFim());
        parada.setCidade(request.getCidade());
        parada.setLocal(request.getLocal());
        ...
        if (request.getManutencao() != null) {
            ...
            manutencao.setCodigo("MAN-" + UUID.randomUUID());
